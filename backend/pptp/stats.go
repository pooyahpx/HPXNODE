...
func (o *PPTP) pollLoop(ctx context.Context) {
...
func (o *PPTP) poll() {
	...
	perUser := make(map[string][]pptpSession)
...
func (o *PPTP) enforceDeviceLimits(perUser map[string][]pptpSession) {
	for user, list := range perUser {
		...
		for _, s := range list[limit:] {
			if s.pid > 0 && terminateSession(s.pid) {
				o.emitLogf("Info", "pptp: user %s over device limit (%d > %d), disconnecting %s",
...
func (o *PPTP) GetStats(ctx context.Context, request *common.StatRequest) (*common.StatResponse, error) {
	o.mu.RLock()
	state := o.state
	o.mu.RUnlock()
	...
	default:
		return nil, errors.New("unsupported stat type for pptp")
	}
}

func (o *PPTP) GetUserOnlineStats(ctx context.Context, email string) (*common.OnlineStatResponse, error) {
	o.mu.RLock()
	state := o.state
	o.mu.RUnlock()
	if state != lifecycleRunning {
...
func (o *PPTP) GetUserOnlineIpListStats(ctx context.Context, email string) (*common.StatsOnlineIpListResponse, error) {
	o.mu.RLock()
	state := o.state
	o.mu.RUnlock()
	if state != lifecycleRunning {
...
func (o *PPTP) GetSysStats(ctx context.Context) (*common.BackendStatsResponse, error) {
	o.mu.RLock()
	state := o.state
	o.mu.RUnlock()
	if state != lifecycleRunning {
